...
public class PRDActionValidator {
    ...
    private boolean checkSingleAction(PRDAction action, PRDWorld oldWorld){
            ...
            switch (action.getType()) {
                ...
                case "condition": {
                    ...
                    if(action.getPRDThen() != null){
                        for(PRDAction actionInThen : action.getPRDThen().getPRDAction()){
                            if(!checkSingleAction(actionInThen, oldWorld)){
                                return false;
                            }
                        }
                    }
    ...
    public boolean allActionsAreValid(PRDActions actions, PRDWorld oldWorld) {
        ...
        for (PRDAction action : actions.getPRDAction()) {
            if(!checkSingleAction(action, oldWorld)){
                return false;
            }
        }
    ...
    private boolean valueInEvaluationIsNumeric(PRDWorld oldWorld, String insideEvaluate) {
        ...
        if (parts.length == 2) {
            ...
            PRDProperty prop = getPropertyByEntityNameAndPropName(propName, entName, oldWorld);
            if (!prop.getType().equals("float")) {
                errorMessage = "Checking percent+evaluate, and I see that property: " + propName + " is not a float. \n";
                return false;
            }
        }
        return true;
